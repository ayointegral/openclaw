...
// SearXNG base URL must be provided via config (tools.web.search.searxng.baseUrl)
// or SEARXNG_BASE_URL env var. No hardcoded default.
...
async function runSearxngSearch(params: {
  ...
  // SearXNG may return many results; trim to requested count
  if (data.results && data.results.length > params.count) {
    data.results = data.results.slice(0, params.count);
  }
  return data;
}
    ...
    params.provider === "brave"
      ...
      : params.provider === "perplexity"
        ...
        : params.provider === "searxng"
          ? `${params.provider}:${params.query}:${params.count}:${params.searxngBaseUrl}:${params.searxngCategories ?? "general"}`
  ...
  const description =
    provider === "perplexity"
      ...
      : provider === "grok"
        ...
        : provider === "searxng"
          ? "Search the web using a self-hosted SearXNG metasearch engine. Aggregates results from multiple search engines. Returns titles, URLs, and snippets for fast research."
